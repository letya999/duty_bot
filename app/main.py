...
if cors_origins_env:
    ...
    # Development fallback - local development only
    allowed_origins = [
        "http://localhost:5173",
        "http://localhost:3000",
        "http://localhost:8000",
    ...
    allow_credentials = True
    logger.warning("CORS configured for development only. Set CORS_ORIGINS environment variable for production.")
...
async def health():
    ...
    return {"status": "ok", "service": "duty_bot"}


@app.get("/health/live")
async def health_live():
    """Liveness probe - checks if service is running (Kubernetes compatible)"""
    return {"status": "alive", "service": "duty_bot"}


@app.get("/health/ready")
async def health_ready():
    """Readiness probe - checks if service is ready to handle requests (Kubernetes compatible)"""
    ...
    if telegram_handler:
        ...
        services["telegram"] = "not_configured"
    ...
    health_status["checks"]["external_services"] = services
...
async def health_detailed():
    ...
    try:
        ...
        health_status["components"]["database"] = {
            "status": "healthy",
            "url": "***" if settings.database_url else "not_configured"
        }
    except Exception as e:
        health_status["components"]["database"] = {
            "status": "unhealthy",
    ...
    health_status["components"]["telegram"] = {
        "status": "configured" if telegram_handler else "not_configured",
        "enabled": bool(settings.telegram_token)
    }

    # Slack
    health_status["components"]["slack"] = {
        "status": "configured" if slack_handler else "not_configured",
    ...
    if scheduled_tasks:
        try:
            jobs = scheduled_tasks.scheduler.get_jobs() if scheduled_tasks.scheduler else []
            health_status["components"]["scheduler"] = {
                "status": "running" if (scheduled_tasks.scheduler and scheduled_tasks.scheduler.running) else "stopped",
                "jobs_count": len(jobs)
            }
        except Exception as e:
            health_status["components"]["scheduler"] = {
